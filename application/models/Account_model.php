...
class Account_model extends CI_Model{
   ...
   public function registerUser($name,$email,$password,$gender,$phonenumber){
     if($this->verifyEmailExists($email,"email") == FALSE){
       ...
       $data = array('name' => $name, 'gender' => $gender, 'phonenumber' => $phonenumber,'email' => $email,'password' => password_hash($password, PASSWORD_DEFAULT),'login_type'=>"email",'date' => date('Y-m-d H:i:s'), 'verified' => '0');
       $this->db->trans_start();
       $this->db->insert('tbl_android_users', $data);
       $insertid = $this->db->insert_id();
       $this->db->trans_complete();
       ...
       if ($insertid){
         $this->status = "ok";
         $this->message = "Registeration successful, Sign in to access Dashboard.";
       }else{
         $this->status = "error";
