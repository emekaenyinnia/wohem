...
class BaseController extends CI_Controller {
	 ...
	 */
	 public function get_data(){
		 ...
		 if(isset($data->email)){
			 $data->email = filter_var($this->cleanup($data->email), FILTER_SANITIZE_EMAIL);
		 }
		 if(isset($data->name)){
			 $data->name = filter_var($this->cleanup($data->name), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
		 }
		 if(isset($data->gender)){
			$data->gender = filter_var($this->cleanup($data->gender), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
		}
		if(isset($data->phonenumber)){
			$data->phonenumber = filter_var($this->cleanup($data->phonenumber), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
		}
		if(isset($data->message)){
			$data->message = filter_var($this->cleanup($data->message), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
		}
	
	
    	if(isset($data->password)){
		     $data->password = filter_var($this->cleanup($data->password), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
		 }
		
		 if(isset($data->token)){
 		     $data->token = filter_var($this->cleanup($data->token), FILTER_SANITIZE_SPECIAL_CHARS);
 		 }
		  if(isset($data->title)){
			$data->title = filter_var($this->cleanup($data->title), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
		}
  
		 if(isset($data->plan)){
 		     $data->plan = filter_var($this->cleanup($data->plan), FILTER_SANITIZE_SPECIAL_CHARS);
 		 }
		 if(isset($data->date)){
 		     $data->date = filter_var($this->cleanup($data->date), FILTER_SANITIZE_SPECIAL_CHARS);
 		 }
 ...
 //Load email library
	 ...
	 $this->email->to($settings->mail_username, $settings->mail_username);
	// $this->email->from('emekaenyinnia@checklos.com', 'emekaenyinnia@checklos.com');
	...
	//  echo $this->email->print_debugger(); die;
	 return $r;
		...
		{ 
			...
			if(isset($data->guestormember)) {
			$data->guestormember = filter_var($this->cleanup($data->guestormember), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
			}
			if(isset($data->messagebox)){
			$data->messagebox = filter_var($this->cleanup($data->messagebox), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
			}	
			if(isset($data->time)){
				$data->time = filter_var($this->cleanup($data->time), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
			}
			if(isset($data->meetingDate)){
				$data->meetingDate = filter_var($this->cleanup($data->meetingDate), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
			}
			if(isset($data->audio)){
				$data->audio = $this->vacleanup($data->audio);
			}
			if(isset($data->video)){
				$data->video = $this->vacleanup($data->video);
			}

			return $data;
			}

			public function vacleanup($data)
			{
				$data = $this->security->xss_clean($data);
				return $data;
			}
