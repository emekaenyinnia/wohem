      ...
      // Datatables Variables
		...
		public function loginUser(){
			...
			if(!empty($data)){
				  ...
				  $password = isset($data->password)?filter_var($data->password, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH):"";
					$packageName = isset($data->packageName)?filter_var($data->packageName, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH):"";
          ...
          if($type!="" && $name!=""){
            $user = $this->account_model->socialLogin($email,$type,$name,$packageName);
					}else{
			 ...
			 if(empty($this->account_model->user)){
				$this->response("error", "Incorrect user name or password");
				exit;
			}
			 $dataArray = array("message" => $this->account_model->message,"user" => $this->account_model->user);
			 $this->response($this->account_model->status,  $dataArray);
			 exit;
		}
		...
		public function registerUser(){
			...
			if(!empty($data)){
				  $name = isset($data->name)?filter_var($data->name, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH):"";
					$email = isset($data->email)?filter_var($data->email, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH):"";
					$phonenumber = isset($data->phonenumber)?filter_var($data->phonenumber, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH):"";
					$gender = isset($data->gender)?filter_var($data->gender, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH):"";
					$password = isset($data->password)?filter_var($data->password, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH):"";

					//check for empty or invalid fields
					$_name_error = $name==""?"Name is empty!":"";
					$_phonenumber_error = $phonenumber==""?"Phone Number is empty!":"";
					$_gender_error = $gender==""?"Gender is empty!":"";
					$_email_error = $email!=""?"":"Email Address  Is not valid!";
					$_password_error = $password==""?"Password is empty!":"";
             ...
             exit;
					...
					$this->account_model->registerUser($name,$email,$password, $gender,$phonenumber);
